...
-- Create filewallball user if not exists and grant permissions (same password as root)
CREATE USER IF NOT EXISTS 'filewallball'@'%' IDENTIFIED BY 'FileWallBall_Root_2025';
GRANT ALL PRIVILEGES ON filewallball_db.* TO 'filewallball'@'%';

-- Create filewallball user for localhost access
CREATE USER IF NOT EXISTS 'filewallball'@'localhost' IDENTIFIED BY 'FileWallBall_Root_2025';
GRANT ALL PRIVILEGES ON filewallball_db.* TO 'filewallball'@'localhost';
