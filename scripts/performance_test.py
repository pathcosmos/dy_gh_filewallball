...
from app.models.orm_models import Base, FileCategory, FileInfo, FileTag, FileTagRelation
...
class PerformanceTester:
    ...
    def _create_categories(self) -> List[FileCategory]:
        ...
        for category in categories:
            existing = (
                ...
                .filter(FileCategory.name == category.name)
                .first()
            )
            if not existing:
    ...
    def _create_tags(self) -> List[FileTag]:
        ...
        for tag_name in tag_names:
            ...
            if not existing:
                tag = FileTag(
                    tag_name=tag_name, tag_color=f"#{random.randint(0, 0xFFFFFF):06x}"
                )
    ...
    def _test_complex_search(self) -> Dict[str, Any]:
        ...
        query, params = self.optimizer.optimize_file_search_query(
            extension="pdf", is_public=True, limit=100
        )
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        for table_name, data in report["tables"].items():
            ...
            if stats:
                print(
                    f"{table_name}: {stats.get('table_rows', 0):,}행, "
...
def main():
    ...
    parser.add_argument(
        "--file-count", type=int, default=10000, help="생성할 테스트 파일 수"
    )
    parser.add_argument(
        "--skip-data-generation", action="store_true", help="데이터 생성 건너뛰기"
    )
    ...
    try:
        ...
        for test_name, result in test_results["tests"].items():
            if "error" not in result:
                print(
                    f"{test_name}: {result['execution_time_ms']:.2f}ms "
