...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_project_keys_project_name"),
        "project_keys",
        ["project_name"],
        unique=False,
    )
    op.create_index(
        op.f("ix_project_keys_project_key"),
        "project_keys",
        ["project_key"],
        unique=True,
    )
    op.create_index(
        op.f("ix_project_keys_request_date"),
        "project_keys",
        ["request_date"],
        unique=False,
    )
    op.create_index(
        op.f("ix_project_keys_request_ip"), "project_keys", ["request_ip"], unique=False
    )
    op.create_index(
        op.f("ix_project_keys_is_active"), "project_keys", ["is_active"], unique=False
    )

    # Add project_key_id column to files table
    op.add_column("files", sa.Column("project_key_id", sa.Integer(), nullable=True))
    op.create_index(
        op.f("ix_files_project_key_id"), "files", ["project_key_id"], unique=False
    )
