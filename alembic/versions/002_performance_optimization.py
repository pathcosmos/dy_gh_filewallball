...
def upgrade() -> None:
    ...
    # 파일-태그 관계 검색 최적화
    op.create_index(
        'ix_file_tag_relations_file_tag', 
        'file_tag_relations', 
        ['file_id', 'tag_id']
    )
    ...
    # 시스템 설정 검색 최적화
    op.create_index(
        'ix_system_settings_public_key', 
        'system_settings', 
        ['is_public', 'setting_key']
    )
    ...
    # 카테고리 검색 성능 최적화
    op.create_index(
        'ix_file_categories_active_name', 
        'file_categories', 
        ['is_active', 'name']
    )
