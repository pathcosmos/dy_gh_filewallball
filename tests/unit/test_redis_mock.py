    ...
    try:
        ...
        with patch("app.services.redis_connection_manager.redis.Redis") as mock_redis:
            ...
            config = ConnectionConfig(
                mode=RedisMode.STANDALONE, nodes=nodes, pool_size=10, max_connections=20
            )
            ...
            print(
                f"📊 연결 정보: {json.dumps(connection_info, indent=2, ensure_ascii=False)}"
            )
...
async def test_cache_service_mock():
    ...
    try:
        ...
        with patch("app.services.cache_service.get_redis_manager") as mock_get_manager:
            ...
            mock_manager.execute_with_retry = lambda func, *args, **kwargs: func(
                *args, **kwargs
            )
            ...
            if (
                retrieved_value
                and retrieved_value.get("message") == test_value["message"]
            ):
            ...
            print(
                f"   결과: {'✅ 정상 (None 반환)' if after_delete is None else '❌ 오류'}"
            )
...
async def test_file_cache_mock():
    ...
    try:
        ...
        with patch("app.services.cache_service.get_redis_manager") as mock_get_manager:
            ...
            print(
                f"   결과: {'✅ 정상 (None 반환)' if after_invalidate is None else '❌ 오류'}"
            )
