...
async def test_metadata_service():
    ...
    test_cases = [
        {
            ...
            "metadata": {"is_public": True, "description": "테스트 파일입니다."},
        },
        {
    ...
    try:
        ...
        # Mock 태그 설정
        mock_tags = [MockFileTag(1, "테스트"), MockFileTag(2, "텍스트")]

        # Mock 카테고리 설정
        mock_category = MockFileCategory(id=1, name="일반", description="일반 파일")
        ...
        if metadata:
            ...
            print(
                f"   카테고리: {metadata['category']['name'] if metadata['category'] else '없음'}"
            )
        else:
            print("❌ 메타데이터 조회 실패")
