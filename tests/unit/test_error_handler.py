...
async def test_error_handler():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 에러 처리 서비스 생성
        error_handler = ErrorHandler(mock_db, temp_dir)
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                if result["error_type"] == test_case["expected_type"].value:
                    ...
                    print(
                        f"   ❌ 에러 타입 불일치: 예상 {test_case['expected_type'].value}, 실제 {result['error_type']}"
                    )
                ...
                if result["status_code"] == test_case["expected_status"]:
                    ...
                    print(
                        f"   ❌ 상태 코드 불일치: 예상 {test_case['expected_status']}, 실제 {result['status_code']}"
                    )
