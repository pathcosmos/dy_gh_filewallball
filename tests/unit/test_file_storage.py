        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                result = await storage_service.save_file(
                    mock_file, test_case["filename"]
                )

                # 결과 확인
                if result["is_duplicate"] == test_case["expected_duplicate"]:
                    print(
                        f"✅ 통과: 예상 {test_case['expected_duplicate']}, 실제 {result['is_duplicate']}"
                    )
                else:
                    print(
                        f"❌ 실패: 예상 {test_case['expected_duplicate']}, 실제 {result['is_duplicate']}"
                    )
