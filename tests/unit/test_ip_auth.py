...
from app.models.orm_models import AllowedIP, Base, IPAuthLog, IPRateLimit
...
def test_api_endpoints():
    ...
    try:
        ...
        with open("test_upload.txt", "rb") as f:
            ...
            response = requests.post(
                f"{base_url}/api/v1/ip-auth/upload", files=files, headers=headers
            )
        ...
        response = requests.delete(
            f"{base_url}/api/v1/ip-auth/allowed-ips",
            params={"ip_address": test_ip, "encryption_key": encryption_key},
        )
