...
class Task6IntegrationTest:
    ...
    def test_file_validation_integration(self):
        ...
        try:
            ...
            with (
                patch.object(validator, "_check_file_extension", return_value=True),
    ...
    def test_file_storage_integration(self):
        ...
        try:
            ...
            if duplicate_result and duplicate_result.get("success"):
                ...
                self.log_test(
                    "중복 파일 처리", "FAIL", f"중복 처리 실패: {duplicate_result}"
                )
            ...
            if stats:
                self.log_test(
                    "저장소 통계", "PASS", f"파일 수: {stats.get('file_count', 0)}"
                )
            else:
    ...
    def test_metadata_integration(self):
        ...
        try:
            ...
            with (
                patch.object(self.db_session, "add"),
    ...
    def test_ip_auth_integration(self):
        ...
        try:
            ...
            with (
                ...
                self.log_test(
                    "허용 IP 추가", "PASS" if result else "FAIL", f"추가 결과: {result}"
                )
            ...
            self.log_test(
                "잘못된 키 거부",
                "PASS" if not wrong_key_result else "FAIL",
    ...
    def test_cache_integration(self):
        ...
        try:
            ...
            with patch.object(
                cache_service.redis_manager, "execute_with_retry"
            ) as mock_execute:
                ...
                self.log_test(
                    "캐시 저장", "PASS" if result else "FAIL", f"저장 결과: {result}"
                )
            ...
            with patch.object(
                ...
                if (
                    retrieved_data
                    and retrieved_data.get("file_uuid") == "test-uuid-123"
                ):
                    self.log_test(
                        "캐시 조회",
                        "PASS",
            ...
            with patch.object(
                ...
                self.log_test(
                    "파일 정보 캐시",
                    "PASS" if cache_result else "FAIL",
                        ...
                        with (
                            ...
                            if metadata_result:
                                ...
                                with patch.object(
                                    ...
                                    cache_result = cache_service.set_file_info(
                                        file_uuid, metadata
                                    )
