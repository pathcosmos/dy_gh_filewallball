...
def optimize_pool_config(
    environment: str, workload_type: str = "default"
) -> Dict[str, Any]:
    """
    ...
    if workload_type == "high_concurrency":
        ...
        base_config["health_check_interval"] = max(
            base_config["health_check_interval"] // 2, 10
        )

    elif workload_type == "low_latency":
        base_config["socket_timeout"] = max(base_config["socket_timeout"] * 0.5, 1.0)
        base_config["socket_connect_timeout"] = max(
            base_config["socket_connect_timeout"] * 0.5, 0.5
        )
        base_config["health_check_interval"] = min(
            base_config["health_check_interval"] * 2, 120
        )
