...
from sqlalchemy import create_engine, text, MetaData, Table, Column, Integer, String, Boolean, DateTime, BigInteger, Text, SmallInteger, Enum
...
system_settings = Table(
    ...
    Column('setting_type', Enum('string', 'integer', 'boolean', 'json'), nullable=False),
    Column('description', Text),
...
class FileModel:
    ...
    def get_file_by_uuid(self, file_uuid: str) -> Optional[Dict[str, Any]]:
        """UUID로 파일 조회"""
        try:
            query = files.select().where(files.c.file_uuid == file_uuid, files.c.is_deleted == False)
            result = self.db.execute(query)
            row = result.fetchone()
    ...
    def get_files(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """파일 목록 조회"""
        try:
            query = files.select().where(files.c.is_deleted == False).order_by(files.c.created_at.desc()).limit(limit).offset(offset)
            result = self.db.execute(query)
    ...
    def delete_file(self, file_uuid: str) -> bool:
        """파일 삭제 (소프트 삭제)"""
        try:
            query = files.update().where(files.c.file_uuid == file_uuid).values(is_deleted=True)
            result = self.db.execute(query)
            self.db.commit()
