...
class DatabaseHelpers:
    ...
    def create_file_with_metadata(
        self, file_data: Dict[str, Any], tags: Optional[List[str]] = None
    ) -> Optional[FileInfo]:
        """파일 정보 생성 및 메타데이터 설정"""
        try:
            # UUID 생성
            ...
            # 파일 해시 생성 (있는 경우)
            if "file_hash" not in file_data and "file_content" in file_data:
    ...
    def find_file_by_hash(self, file_hash: str) -> Optional[FileInfo]:
        ...
        try:
            return (
                ...
                .filter(FileInfo.file_hash == file_hash, not FileInfo.is_deleted)
                .first()
            )
        except Exception as e:
    ...
    def search_files(
        ...
        try:
            # 기본 쿼리
            query_obj = self.db_session.query(FileInfo).filter(
            ...
            if tags:
                query_obj = (
                    query_obj.join(FileTagRelation)
                    .join(FileTag)
                    .filter(FileTag.tag_name.in_(tags), FileTag.is_active == True)
                )
        ...
        try:
            for tag_name in tag_names:
                ...
                tag = (
                    ...
                    .filter(FileTag.tag_name == tag_name)
                    .first()
                )
    ...
    def get_files_by_tags(
        self, tag_names: List[str], limit: int = 100
    ) -> List[FileInfo]:
        """태그로 파일 검색"""
        try:
            return (
    ...
    def get_popular_tags(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [{"tag_name": row[0], "file_count": row[1]} for row in result]
        except Exception as e:
    ...
    def get_file_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # 전체 파일 수
            total_files = (
                self.db_session.query(func.count(FileInfo.id))
            ...
            # 전체 파일 크기
            total_size = (
                self.db_session.query(func.sum(FileInfo.file_size))
                .filter(FileInfo.is_deleted == False)
                .scalar()
                or 0
            )
            ...
            extension_stats = (
                ...
                .filter(FileInfo.is_deleted == False)
                .group_by(FileInfo.file_extension)
                .order_by(desc("file_count"))
                .limit(10)
                .all()
            )
    ...
    def get_upload_trends(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = (
                ...
                .group_by(func.date(FileInfo.created_at))
                .order_by(asc("date"))
                .all()
            )
    ...
    def get_file_activity_stats(self, file_uuid: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # 조회 통계
            view_count = (
                self.db_session.query(func.count(FileView.id))
                .filter(
                    FileView.file_id == file_info.id, FileView.created_at >= start_date
                )
                .scalar()
            )
            ...
            # 다운로드 통계
            download_count = (
                self.db_session.query(func.count(FileDownload.id))
                .filter(
            ...
            # 최근 활동
            recent_views = (
                self.db_session.query(FileView)
                .filter(FileView.file_id == file_info.id)
                .order_by(desc(FileView.created_at))
                .limit(10)
                .all()
            )

            recent_downloads = (
                self.db_session.query(FileDownload)
                .filter(FileDownload.file_id == file_info.id)
                .order_by(desc(FileDownload.created_at))
                .limit(10)
                .all()
            )
    ...
    def get_system_setting(self, key: str, default: Any = None) -> Any:
        ...
        try:
            setting = (
                self.db_session.query(SystemSetting)
            ...
            if setting:
                ...
                if setting.setting_type == "integer":
                    return (
                        int(setting.setting_value) if setting.setting_value else default
                    )
    ...
    def cleanup_old_logs(self, days: int = 90) -> int:
        ...
        try:
            ...
            view_deleted = (
                self.db_session.query(FileView)
                .filter(FileView.created_at < cutoff_date)
                .delete()
            )
            ...
            download_deleted = (
                self.db_session.query(FileDownload)
                .filter(FileDownload.created_at < cutoff_date)
                .delete()
            )
            ...
            upload_deleted = (
                ...
                .filter(FileUpload.created_at < cutoff_date)
                .delete()
            )
