...
class PerformanceAnalyzer:
    ...
    def get_slow_queries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                f"슬로우 쿼리 로그 조회 실패 (slow query log가 활성화되지 않음): {e}"
            )
            return []
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        report = {"generated_at": time.time(), "tables": {}, "recommendations": []}
        ...
        for table_name, data in tables_data.items():
            ...
            if index_count > 10:
                recommendations.append(
                    f"{table_name}: 인덱스가 많음 - 불필요한 인덱스 제거 고려"
                )
            ...
            for idx in indexes:
                if idx.get("cardinality", 0) < 10:
                    recommendations.append(
                        f"{table_name}.{idx['index_name']}: 낮은 카디널리티 - 인덱스 제거 고려"
                    )
...
class QueryOptimizer:
    ...
    def optimize_file_search_query(
        self,
    ...
    def optimize_tag_search_query(
        ...
        params = {"tag_names": tuple(tag_names), "limit": limit}
