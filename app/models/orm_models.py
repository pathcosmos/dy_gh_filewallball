...
from sqlalchemy import (
    Column, Integer, String, Boolean, DateTime, BigInteger, Text, 
    SmallInteger, Enum, ForeignKey, UniqueConstraint, Float
)
...
class FileCategory(Base):
    ...
    # 관계 정의
    files: Mapped[List["FileInfo"]] = relationship("FileInfo", back_populates="category")
...
class FileExtension(Base):
    ...
    extension: Mapped[str] = mapped_column(String(10), unique=True, nullable=False, index=True)
    mime_type: Mapped[str] = mapped_column(String(100), nullable=False)
...
class FileUpload(Base):
    ...
    upload_started_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class FileTag(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    tag_name: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, index=True)
    ...
    # 관계 정의
    files: Mapped[List["FileInfo"]] = relationship("FileInfo", secondary="file_tag_relations", back_populates="tags")
...
class FileTagRelation(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    file_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("files.id"), nullable=False)
    tag_id: Mapped[int] = mapped_column(Integer, ForeignKey("file_tags.id"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class FileView(Base):
    ...
    view_type: Mapped[str] = mapped_column(Enum('info', 'preview', 'download'), nullable=False)
    session_id: Mapped[Optional[str]] = mapped_column(String(100))
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    
    def __repr__(self):
        return f"<FileView(id={self.id}, file_id={self.file_id}, type='{self.view_type}')>"
...
class FileDownload(Base):
    ...
    user_agent: Mapped[Optional[str]] = mapped_column(Text)
    download_method: Mapped[str] = mapped_column(Enum('direct', 'api', 'web'), nullable=False)
...
class SystemSetting(Base):
    ...
    setting_type: Mapped[str] = mapped_column(String(20), default="string")  # string, integer, boolean, json
    description: Mapped[Optional[str]] = mapped_column(String(255))
...
def add_tags_to_file(db_session, file_id: int, tag_names: List[str]) -> bool:
    ...
    try:
        for tag_name in tag_names:
            ...
            # 관계가 이미 존재하는지 확인
            existing_relation = db_session.query(FileTagRelation).filter(
...
def remove_tags_from_file(db_session, file_id: int, tag_names: List[str]) -> bool:
    ...
    try:
        for tag_name in tag_names:
            tag = db_session.query(FileTag).filter(FileTag.tag_name == tag_name).first()
            if tag:
                relation = db_session.query(FileTagRelation).filter(
                    FileTagRelation.file_id == file_id,
                    FileTagRelation.tag_id == tag.id
                ).first()
                if relation:
...
def soft_delete_file(db_session, file_uuid: str) -> bool:
    """파일 소프트 삭제"""
    try:
        file_info = db_session.query(FileInfo).filter(
...
class AllowedIP(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ip_address: Mapped[str] = mapped_column(String(45), nullable=False, index=True)  # IPv6 지원
    ...
    key_hash: Mapped[str] = mapped_column(String(255), nullable=False, index=True)  # 키 해시 저장
    ...
    # 관계 정의
    auth_logs: Mapped[List["IPAuthLog"]] = relationship("IPAuthLog", back_populates="allowed_ip")
    
    __table_args__ = (UniqueConstraint('ip_address', 'encryption_key', name='unique_ip_key'),)
...
class IPAuthLog(Base):
    ...
    allowed_ip_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("allowed_ips.id"))
    api_key_hash: Mapped[Optional[str]] = mapped_column(String(255), index=True)
    action: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # upload, auth_failed, rate_limited
    ...
    # 관계 정의
    allowed_ip: Mapped[Optional["AllowedIP"]] = relationship("AllowedIP", back_populates="auth_logs")
    
    def __repr__(self):
        return f"<IPAuthLog(id={self.id}, ip='{self.ip_address}', action='{self.action}')>"
