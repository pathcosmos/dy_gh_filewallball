...
from fastapi import (
    APIRouter, Depends, HTTPException, status, Request, Response, Query
)
...
from app.models.orm_models import (
    FileInfo, FileView, FileDownload, FileTag, FileTagRelation
)
...
async def list_files(
    ...
    sort_by: str = Query("created_at", description="정렬 기준 (filename, file_size, created_at, updated_at)"),
    sort_order: str = Query("desc", description="정렬 순서 (asc, desc)"),
    category_id: Optional[int] = Query(None, description="카테고리 ID로 필터링"),
    tags: Optional[str] = Query(None, description="태그로 필터링 (쉼표로 구분된 문자열)"),
    file_type: Optional[str] = Query(None, description="파일 확장자로 필터링 (예: jpg, pdf)"),
    ...
    try:
        ...
        if tags:
            ...
            if tag_list:
                # 태그 관계를 통한 필터링
                query = query.join(FileTagRelation).join(FileTag).filter(
                    FileTag.tag_name.in_(tag_list)
                ).distinct()
        ...
        for file_info in files:
            file_data = {
                ...
                } if file_info.category else None,
                "tags": [
                    {
                        "id": tag.id,
                        "name": tag.tag_name,
...
async def search_files(
    ...
    try:
        ...
        # 기본 쿼리 구성
        search_query = db.query(FileInfo).filter(
            FileInfo.is_deleted == False
        ).options(
            joinedload(FileInfo.category),
            joinedload(FileInfo.tags)
        ...
        if date_from:
            search_query = search_query.filter(
                FileInfo.created_at >= date_from
            )
        if date_to:
            search_query = search_query.filter(
                FileInfo.created_at <= date_to
            )
        ...
        # 정렬 및 페이지네이션 적용
        files = search_query.order_by(FileInfo.created_at.desc()).offset(offset).limit(size).all()
...
async def get_file_info(
    ...
    try:
        ...
        response_data = {
            ...
            "statistics": stats_data if stats_data else {
                "total_views": 0,
                "unique_viewers": 0,
                "total_downloads": 0,
...
async def download_file(
    ...
    try:
        ...
        logger.info(f"File download started for UUID: {file_uuid}, size: {file_info.file_size}")
...
async def update_file_info(
    ...
    try:
        ...
        # 업데이트 가능한 필드들
        allowed_fields = {
            'original_filename', 'description', 'is_public', 
            'file_category_id', 'tags'
        }
        ...
        try:
            ...
            logger.warning(f"캐시 무효화 실패 (무시됨): {file_uuid}, 오류: {cache_error}")
...
async def delete_file(
    ...
    try:
        ...
        can_delete, reason = rbac_service.can_access_file(current_user, file_info, 'delete')
        if not can_delete:
            raise HTTPException(
...
async def permanent_delete_file(
    ...
    try:
        ...
        can_permanent_delete, reason = rbac_service.can_access_file(current_user, file_info, 'permanent_delete')
