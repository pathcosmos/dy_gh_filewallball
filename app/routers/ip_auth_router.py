    ...
    try:
        ...
        if not allowed_ip:
            ip_auth_service.log_auth_event(
                client_ip, x_api_key, "auth_failed",
        ...
        if not ip_auth_service.check_rate_limit(client_ip, x_api_key):
            ip_auth_service.log_auth_event(
                client_ip, x_api_key, "rate_limited",
                user_agent=request.headers.get("User-Agent"),
                response_code=429,
        ...
        if allowed_ip.max_file_size and file.size > allowed_ip.max_file_size:
            ip_auth_service.log_auth_event(
                client_ip, x_api_key, "file_too_large",
                user_agent=request.headers.get("User-Agent"),
                response_code=413,
        ...
        if 'ip_auth_service' in locals():
            ip_auth_service.log_auth_event(
                client_ip, x_api_key, "upload_error",
...
async def add_allowed_ip(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"허용 IP 추가 중 오류가 발생했습니다: {str(e)}"
        )
...
async def remove_allowed_ip(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404,
                detail="지정된 IP와 키 조합을 찾을 수 없습니다"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"허용 IP 제거 중 오류가 발생했습니다: {str(e)}"
        )
...
async def regenerate_encryption_key(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"키 재생성 중 오류가 발생했습니다: {str(e)}"
        )
