...
class RateLimiterService:
    ...
    def is_rate_limited(
        self, 
        client_ip: str, 
        limit_type: str = 'uploads_per_hour'
    ) -> Tuple[bool, Dict]:
        """
        Rate limiting 확인
        ...
        try:
            ...
            limit_info = {
                'limit': limit,
                'remaining': remaining,
                'reset_time': window_start + 3600 if limit_type == 'uploads_per_hour' else window_start + 86400,
    ...
    def increment_request_count(self, client_ip: str, limit_type: str = 'uploads_per_hour'):
        """요청 카운트 증가"""
        try:
            current_time = int(time.time())
    ...
    def get_upload_statistics(self, client_ip: str, days: int = 7) -> Dict:
        ...
        try:
            ...
            daily_stats = self.db_session.query(
                ...
                func.date(FileUpload.upload_time)
            ).all()
            
            # 전체 통계
            total_uploads = self.db_session.query(func.count(FileUpload.id)).filter(
                FileUpload.upload_ip == client_ip,
                FileUpload.upload_time >= start_date,
                FileUpload.upload_time <= end_date
            ).scalar()
            
            total_size = self.db_session.query(func.sum(FileUpload.file_size)).filter(
                FileUpload.upload_ip == client_ip,
                FileUpload.upload_time >= start_date,
                FileUpload.upload_time <= end_date
            ).scalar() or 0
    ...
    def get_rate_limit_headers(self, client_ip: str) -> Dict[str, str]:
        ...
        try:
            # 시간별 제한 확인
            is_hour_limited, hour_info = self.is_rate_limited(client_ip, 'uploads_per_hour')
            is_day_limited, day_info = self.is_rate_limited(client_ip, 'uploads_per_day')
