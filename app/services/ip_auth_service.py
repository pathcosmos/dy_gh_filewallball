...
class IPAuthService:
    ...
    def verify_ip_and_key(self, client_ip: str, api_key: str) -> Optional[AllowedIP]:
        ...
        try:
            ...
            # 활성화된 IP 허용 목록에서 검색
            allowed_ip = self.db.query(AllowedIP).filter(
                and_(
            ...
            self._log_auth_event(client_ip, api_key, "auth_failed", 
                               error_message=f"검증 오류: {str(e)}")
            return None
    ...
    def check_rate_limit(self, client_ip: str, api_key: str) -> bool:
        ...
        try:
            ...
            # 현재 시간대의 요청 수 확인
            rate_limit = self.db.query(IPRateLimit).filter(
                and_(
            ...
            self.db.rollback()
            self._log_auth_event(client_ip, api_key, "rate_limit_error", 
                               error_message=f"Rate limit 오류: {str(e)}")
            return False
    
    def log_auth_event(self, client_ip: str, api_key: str, action: str, 
                      file_uuid: Optional[str] = None, user_agent: Optional[str] = None,
                      response_code: Optional[int] = None, error_message: Optional[str] = None,
                      request_size: Optional[int] = None, processing_time_ms: Optional[int] = None) -> None:
        """인증 이벤트 로깅"""
        try:
    ...
    def _log_auth_event(self, client_ip: str, api_key: str, action: str, 
                       error_message: Optional[str] = None) -> None:
        """내부 로깅 메서드"""
        self.log_auth_event(client_ip, api_key, action, error_message=error_message)
    
    def get_ip_statistics(self, ip_address: Optional[str] = None, 
        ...
        try:
            ...
            successful_uploads = query.filter(
                and_(
                    IPAuthLog.action == "upload",
                    IPAuthLog.response_code == 200
                )
            ).count()
            
            # 실패한 요청 수
            failed_requests = query.filter(
                IPAuthLog.response_code.isnot(None)
            ).filter(
                IPAuthLog.response_code >= 400
            ).count()
            
            # Rate limited 요청 수
            rate_limited = query.filter(
                IPAuthLog.action == "rate_limited"
            ).count()
            ...
            daily_stats = self.db.query(
                ...
                func.sum(func.case([(IPAuthLog.action == "upload", 1)], else_=0)).label('uploads'),
                func.sum(func.case([(IPAuthLog.response_code >= 400, 1)], else_=0)).label('errors')
            ).filter(
                IPAuthLog.request_time >= start_date
            ).group_by(
                func.date(IPAuthLog.request_time)
            ).order_by(
                func.date(IPAuthLog.request_time)
            ).all()
            ...
            return {
                ...
                "success_rate": (successful_uploads / total_requests * 100) if total_requests > 0 else 0,
                "daily_stats": [
                    {
                        "date": str(stat.date),
                        "total": stat.total,
    ...
    def add_allowed_ip(self, ip_address: str, encryption_key: str, 
                      ip_range: Optional[str] = None, permissions: Optional[Dict] = None,
                      max_uploads_per_hour: int = 100, max_file_size: int = 104857600,
                      expires_at: Optional[datetime] = None) -> AllowedIP:
        """허용된 IP 추가"""
        try:
            # IP 주소 유효성 검사
            ...
            # 중복 확인
            existing = self.db.query(AllowedIP).filter(
                and_(
