...
class CacheFallbackService:
    ...
    async def get_with_fallback(
        self, key: str, fetch_func: Callable, cache_type: str = "default"
    ) -> Optional[Any]:
        """
        캐시 조회 (폴백 포함)
    ...
    async def _fallback_to_database(
        self, key: str, fetch_func: Callable
    ) -> Optional[Any]:
        """데이터베이스 폴백 조회"""
    ...
    async def _fetch_from_database(
        ...
        try:
            ...
            # 비동기 타임아웃 적용
            db_value = await asyncio.wait_for(fetch_func(key), timeout=timeout)
        ...
        if self.circuit_breaker["state"] != CircuitBreakerState.OPEN:
            ...
            await self._send_alert(
                "circuit_breaker_opened",
                {
                    ...
                    "last_failure_time": self.circuit_breaker[
                        "last_failure_time"
                    ].isoformat(),
                },
            )
    ...
    async def _close_circuit_breaker(self):
        ...
        # 복구 완료 알림
        await self._send_alert(
            "circuit_breaker_closed", {"recovery_time": datetime.utcnow().isoformat()}
        )
    ...
    async def create_cache_backup(self) -> bool:
        ...
        try:
            ...
            # 백업 파일 저장
            backup_filename = (
                f"cache_backup_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            fallback_timeout = (
                fallback_duration
                and fallback_duration > self.fallback_config["max_fallback_duration"]
            )

            return {
                "status": (
                    "healthy"
                    if circuit_breaker_healthy and not fallback_timeout
                    else "warning"
                ),
