...
from app.models.orm_models import (
    FileInfo, FileUpload, FileTag, FileTagRelation, 
    FileCategory, SystemSetting
)
...
class MetadataService:
    ...
    async def _validate_category(self, category_id: int) -> None:
        ...
        """
        category = self.db_session.query(FileCategory).filter(
        ...
        if not category:
            raise HTTPException(
                status_code=400,
                detail=f"존재하지 않는 카테고리입니다: {category_id}"
            )
    ...
    def get_file_metadata(self, file_uuid: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 업로드 기록 조회
            upload_record = self.db_session.query(FileUpload).filter(
                FileUpload.file_uuid == file_uuid
            ).order_by(FileUpload.upload_time.desc()).first()
            
            # 태그 조회
            tags = self.db_session.query(FileTag).join(FileTagRelation).filter(
                FileTagRelation.file_uuid == file_uuid
            ).all()
            ...
            if file_info.category_id:
                category = self.db_session.query(FileCategory).filter(
                    FileCategory.id == file_info.category_id
                ).first()
            ...
            return {
                ...
                'is_public': file_info.is_public,
                'description': file_info.description,
                'upload_ip': upload_record.upload_ip if upload_record else None,
                'upload_time': upload_record.upload_time.isoformat() if upload_record else None,
    ...
    def update_file_metadata(
        ...
        try:
            ...
            # 업데이트 가능한 필드들
            allowed_fields = {
                'description', 'is_public', 'category_id'
            }
            ...
            if 'tags' in updates:
                ...
                if updates['tags']:
                    asyncio.create_task(
                        self._process_tags(file_uuid, updates['tags'])
                    )
    ...
    def get_upload_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # 총 업로드 수
            total_uploads = self.db_session.query(FileUpload).filter(
                FileUpload.upload_time >= start_date,
                FileUpload.upload_status == 'success'
            ).count()
            
            # 성공한 업로드 수
            successful_uploads = self.db_session.query(FileUpload).filter(
                FileUpload.upload_time >= start_date,
                FileUpload.upload_status == 'success'
            ).count()
            
            # 실패한 업로드 수
            failed_uploads = self.db_session.query(FileUpload).filter(
                FileUpload.upload_time >= start_date,
                FileUpload.upload_status == 'failed'
            ).count()
            
            # 총 파일 크기
            total_size = self.db_session.query(FileInfo).filter(
                FileInfo.created_at >= start_date,
                FileInfo.is_deleted == False
            ).with_entities(
                func.sum(FileInfo.file_size)
            ).scalar() or 0
