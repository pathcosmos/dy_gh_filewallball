...
from app.models.orm_models import FileUpload, FileInfo
...
class ErrorHandlerService:
    ...
    async def handle_upload_error(
        ...
        try:
            ...
            # 2. 에러 로그 기록
            error_info = await self._log_error(error, error_type, file_uuid, request, context)
            ...
            status_code, error_message = self._generate_error_response(error_type, error)
    ...
    async def _log_error(
        ...
        try:
            ...
            error_info = {
                ...
                'user_agent': upload_record.user_agent,
                'timestamp': datetime.now().isoformat(),
                'context': context or {}
            }
    ...
    def _generate_error_response(self, error_type: ErrorType, error: Exception) -> tuple:
        """
        에러 응답 생성
    ...
    async def get_error_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # 에러 타입별 통계
            error_types = self.db_session.query(
                FileUpload.error_type,
                func.count(FileUpload.id)
            ).filter(
                FileUpload.upload_time >= start_date,
                FileUpload.upload_status == 'failed'
            ).group_by(FileUpload.error_type).all()
            
            # 재시도 가능한 에러 수
            retryable_errors = sum(
                count for error_type, count in error_types
    ...
    async def cleanup_old_error_logs(self, days: int = 90) -> int:
        ...
        try:
            ...
            # 오래된 업로드 실패 기록 삭제
            old_records = self.db_session.query(FileUpload).filter(
