...
from app.models.orm_models import FileDownload, FileInfo, FileStatistics, FileView
...
class StatisticsService:
    ...
    async def get_file_statistics(
        ...
        stats_result = (
            db.query(FileStatistics)
            .filter(FileStatistics.file_uuid == file_uuid)
            .first()
        )
        ...
        stats_data = {
            ...
            "last_viewed": (
                stats_result.last_viewed.isoformat()
                if stats_result.last_viewed
                else None
            ),
            "first_viewed": (
                stats_result.first_viewed.isoformat()
                if stats_result.first_viewed
                else None
            ),
            "last_downloaded": (
                stats_result.last_downloaded.isoformat()
                if stats_result.last_downloaded
                else None
            ),
            "first_downloaded": (
                stats_result.first_downloaded.isoformat()
                if stats_result.first_downloaded
                else None
            ),
        ...
        try:
            await self.cache_service.set(
                cache_key, stats_data, self.cache_config["file_stats_ttl"]
            )
            logger.info(f"파일 통계 캐시 저장 완료: {file_uuid}")
        except Exception as cache_error:
            logger.warning(
                f"통계 캐시 저장 실패 (무시됨): {file_uuid}, 오류: {cache_error}"
            )
    ...
    async def get_popular_files(
        ...
        popular_files = (
            ...
            .order_by(desc(FileStatistics.popularity_score))
            .limit(limit)
            .all()
        )
        ...
        for file_stats in popular_files:
            file_data = {
                ...
                "last_viewed": (
                    file_stats.last_viewed.isoformat()
                    if file_stats.last_viewed
                    else None
                ),
        ...
        try:
            await self.cache_service.set(
                cache_key, popular_data, self.cache_config["popular_files_ttl"]
            )
    ...
    async def get_trending_files(
        ...
        trending_files = (
            ...
            .order_by(desc(FileStatistics.recent_views))
            .limit(limit)
            .all()
        )
        ...
        try:
            await self.cache_service.set(
                cache_key, trending_data, self.cache_config["trending_files_ttl"]
            )
    ...
    async def invalidate_file_statistics_cache(self, file_uuid: str) -> bool:
        ...
        try:
            ...
            await self.cache_service.clear_pattern(
                f"{self.cache_config['cache_key_prefix']}popular:*"
            )
            await self.cache_service.clear_pattern(
                f"{self.cache_config['cache_key_prefix']}trending:*"
            )
    ...
    async def get_system_statistics(self, db: Session) -> Dict[str, Any]:
        ...
        total_size = (
            db.query(func.sum(FileInfo.file_size))
            .filter(FileInfo.is_public == True, FileInfo.is_deleted == False)
            .scalar()
            or 0
        )
        ...
        recent_views = (
            db.query(func.count(FileView.id))
            .filter(FileView.created_at >= yesterday)
            .scalar()
        )

        recent_downloads = (
            db.query(func.count(FileDownload.id))
            .filter(FileDownload.created_at >= yesterday)
            .scalar()
        )
        ...
        system_stats = {
            ...
            "avg_views_per_file": (
                round(total_views / total_files, 2) if total_files > 0 else 0
            ),
            "avg_downloads_per_file": (
                round(total_downloads / total_files, 2) if total_files > 0 else 0
            ),
            "engagement_rate": (
                round(total_downloads / total_views * 100, 2) if total_views > 0 else 0
            ),
        ...
        try:
            await self.cache_service.set(
                cache_key, system_stats, self.cache_config["file_stats_ttl"]
            )
        except Exception as cache_error:
