...
class FileValidationService:
    ...
    def __init__(self):
        ...
        self.blocked_extensions = {
            ...
            '.jar', '.war', '.ear', '.class', '.py', '.php', '.pl', '.rb',
            '.sh', '.bash', '.zsh', '.fish', '.ps1', '.psm1', '.psd1',
            '.msi', '.msm', '.msp', '.app', '.dmg', '.deb', '.rpm',
            '.apk', '.ipa', '.xap', '.swf', '.fla', '.flv'
        }
    ...
    def validate_file_size(self, file_size: int) -> Tuple[bool, str]:
        ...
        if file_size > self.max_file_size:
            return False, f"파일 크기가 너무 큽니다. 최대 {self.max_file_size // (1024*1024)}MB까지 허용됩니다."
        
        if file_size < self.min_file_size:
            return False, f"파일 크기가 너무 작습니다. 최소 {self.min_file_size // 1024}KB 이상이어야 합니다."
    ...
    def validate_filename(self, filename: str) -> Tuple[bool, str]:
        ...
        for pattern in self.forbidden_filename_patterns:
            if pattern in filename_lower:
                return False, f"허용되지 않는 파일명 패턴이 포함되어 있습니다: {pattern}"
    ...
    def validate_mime_type(self, file_content: bytes, filename: str) -> Tuple[bool, str, str]:
        ...
        try:
            ...
            if detected_mime in self.blocked_mime_types:
                return False, f"허용되지 않는 파일 타입입니다: {detected_mime}", detected_mime
            
            # 허용된 MIME 타입 검사
            if detected_mime not in self.allowed_mime_types:
                return False, f"지원되지 않는 파일 타입입니다: {detected_mime}", detected_mime
    ...
    async def validate_upload_file(self, file: UploadFile) -> Dict[str, any]:
        ...
        try:
            ...
            # 파일 확장자 검증
            extension_valid, extension_message = self.validate_file_extension(file.filename)
            ...
            # MIME 타입 검증
            mime_valid, mime_message, detected_mime = self.validate_mime_type(file_content, file.filename)
            ...
            if detected_mime and detected_mime.startswith('text/'):
                content_valid, content_message = self.validate_file_content(file_content)
            ...
            validation_result['errors'].append(f"파일 검증 중 오류가 발생했습니다: {str(e)}")
            return validation_result
