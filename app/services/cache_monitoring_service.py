...
class CacheMonitoringService:
    ...
    async def get_percentile_response_time(self, operation: str = 'default', 
    ...
    async def generate_prometheus_metrics(self) -> str:
        ...
        try:
            ...
            for operation in self.response_times.keys():
                ...
                metrics.append(f'cache_response_time_avg{{operation="{operation}"}} {avg_time}')
                metrics.append(f'cache_response_time_p95{{operation="{operation}"}} {p95_time}')
                metrics.append(f'cache_response_time_p99{{operation="{operation}"}} {p99_time}')
            
            # 메모리 사용량 메트릭
            memory_record = await self.collect_memory_metrics()
            if memory_record:
                metrics.append(f'cache_memory_used_bytes {memory_record["used_memory"]}')
                metrics.append(f'cache_memory_peak_bytes {memory_record["used_memory_peak"]}')
                metrics.append(f'cache_memory_max_bytes {memory_record["maxmemory"]}')
                metrics.append(f'cache_memory_usage_percent {memory_record["memory_usage_percent"]}')
                metrics.append(f'cache_keys_total {memory_record["total_keys"]}')
            
            # 알림 상태 메트릭
            for alert_name, is_active in self.alerts.items():
                metrics.append(f'cache_alert_active{{alert="{alert_name}"}} {1 if is_active else 0}')
            ...
            circuit_breaker_state = 1 if connection_info.get('circuit_breaker_state') == 'open' else 0
    ...
    async def generate_cache_efficiency_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 알림 상태
            active_alerts = [name for name, is_active in self.alerts.items() if is_active]
