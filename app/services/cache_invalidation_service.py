...
class CacheInvalidationService:
    ...
    async def publish_invalidation_event(self, event_type: str, target_keys: List[str] = None, 
                                       tags: List[str] = None, channel: str = 'cache_invalidation'):
        """
        무효화 이벤트 발행
    ...
    async def invalidate_file_cache(self, file_uuid: str, event_type: str = 'file_updated'):
        """
        파일 캐시 무효화 (로컬 + 분산)
            ...
            for tag in tags:
                ...
                if tag_key:
                    ...
                    if keys:
                        ...
                        logger.info(f"태그 기반 캐시 무효화 완료: {tag}, {len(keys)}개 키")
    ...
    async def track_cache_operation(self, transaction_id: str, operation: str, 
                                  key: str, value: Any = None, ttl: int = None):
        """
        트랜잭션 중 캐시 작업 추적
    ...
    def _log_invalidation_event(self, event_type: str, target_keys: List[str], 
    ...
    async def get_invalidation_stats(self) -> Dict[str, Any]:
        ...
        try:
            # 최근 무효화 이벤트 분석
            recent_events = self.invalidation_log[-100:] if self.invalidation_log else []
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy" if pubsub_healthy and redis_healthy else "unhealthy",
