...
class SchedulerService:
    ...
    async def cleanup_deleted_files(self):
        ...
        try:
            ...
            deleted_files = (
                ...
                .filter(
                    and_(
                        FileInfo.is_deleted == True, FileInfo.updated_at <= cutoff_date
                    )
                )
                .all()
            )
    ...
    async def _permanently_delete_file(self, file_info: FileInfo, db: Session):
        ...
        try:
            ...
            logger.error(
                f"Failed to permanently delete file {file_info.file_uuid}: {e}"
            )
            raise
    ...
    async def get_cleanup_stats(self, db: Session) -> dict:
        ...
        try:
            ...
            recently_deleted = (
                db.query(FileInfo)
                .filter(
                    and_(FileInfo.is_deleted == True, FileInfo.updated_at >= week_ago)
                )
                .count()
            )
